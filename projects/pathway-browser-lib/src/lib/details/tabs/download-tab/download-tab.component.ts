...
export class DownloadTabComponent {
  newtUrl = computed(() => {
    const reactomeUrl = new URL(`${this.config.CONTENT_SERVICE}/exporter/event/${this.finalEventId()}.sbgn&inferNestingOnLoad=true&mapColorScheme=opposed_red_blue&fitLabelsToNodes=true`, window.location.origin).href
  ...
  pathwayItems: PathwayItem[] = [
    {
      name: 'SBML',
      url: computed(() => `${this.config.CONTENT_SERVICE}/exporter/event/${this.finalEventId()}.sbml`),
    },
    {
      name: 'SBGN',
      url: computed(() => `${this.config.CONTENT_SERVICE}/exporter/event/${this.finalEventId()}.sbgn`),
    },
    {
      name: 'BioPAX2',
      url: computed(() => `${this.config.RESTFUL_API}/biopaxExporter/Level2/${this.biopaxId()}`),
      fileName: computed(() => `${this.biopaxId()}.xml`)
    },
    {
      name: 'BioPAX3',
      url: computed(() => `${this.config.RESTFUL_API}/biopaxExporter/Level3/${this.biopaxId()}`),
      fileName: computed(() => `${this.biopaxId()}.xml`)
    },
    {
      name: 'PDF',
      url: computed(() => {
        const url = `${this.config.CONTENT_SERVICE}/exporter/document/event/${this.finalEventId()}.pdf`;
        const analysisUrl = `${this.config.CONTENT_SERVICE}/exporter/document/event/${this.finalEventId()}.pdf?token=${this.token()}`;
  ...
  analysisItems: AnaLysisItem[] = [
    {
      ...
      url: computed(() => `${this.config.ANALYSIS_SERVICE}/download/${this.token()}/pathways/${this.currentAnalysisResource()}/result.csv`),
      icon: {id:'table'},
...

    {
      ...
      url: computed(() => `${this.config.ANALYSIS_SERVICE}/download/${this.token()}/result.json.gz`),
      icon: {id:'data_object'},
      isShown: signal(true)
    },

    {
      ...
      url: computed(() => `${this.config.ANALYSIS_SERVICE}/report/${this.token()}/${this.currentAnalysisSpecies()}/report.pdf`),
      icon: {id:'docs'},
      isShown: computed(() => !this.analysis.isGSA())
    },

    {
      ...
      url: computed(() => `${this.config.ANALYSIS_SERVICE}/download/${this.token()}/entities/found/${this.currentAnalysisResource()}/mapping.csv`),
      icon: {id:'table'},
      isShown: signal(true)
    },

    {
      ...
      url: computed(() => `${this.config.ANALYSIS_SERVICE}/download/${this.token()}/entities/notfound/not_found.csv`),
      icon: {id:'table'},
      isShown: signal(true)
    }]
  ...
  constructor(private state: UrlStateService,
              ...
              public config: PathwayBrowserConfigService) {
  }
  ...
  getExportUrl(format: string) {
    const analysisUrl = `${this.config.CONTENT_SERVICE}/exporter/diagram/${this.pathwayId()}.${format}?token=${this.token()}`
    const url = `${this.config.CONTENT_SERVICE}/exporter/diagram/${this.pathwayId()}.${format}`
    return this.hasResult() ? analysisUrl : url
  }
