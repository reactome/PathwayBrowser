...
export class UrlStateService implements State {
  ...
  constructor(private route: ActivatedRoute, private router: Router, private http: HttpClient, private config: PathwayBrowserConfigService) {

    this.router.events.pipe(
      filter(event => event instanceof NavigationEnd),
      switchMap(() => this.getPathwayIdFromRoute()),
      map(params => params?.['pathwayId'])
    ).subscribe((id) => {
      this.pathwayId.set(id)
    });

    // Only navigate when pathwayId changes to a defined value
    // Don't navigate on initial undefined to prevent redirecting away from the base route
    let initialized = false;
    effect(() => {
      const id = this.pathwayId();
      console.log('Updating patwhayId to ', id)
      if (id) {
        ...
        // Only navigate to base if we previously had a pathwayId
  ...
  async dbIdToStId(dbId: number): Promise<string> {
    return firstValueFrom(this.http.get(`${this.config.CONTENT_SERVICE}/data/query/${dbId}/stId`, {responseType: "text"}).pipe(
      catchError(() => of(dbId + '')))
    );
  }

  /**
   * Traverses the route tree to find the route with pathwayId parameter.
   * This is needed when the library is loaded as a child route (e.g., /PathwayBrowser/:pathwayId)
   */
  private getPathwayIdFromRoute() {
    let route = this.router.routerState.root;
    while (route.firstChild) {
      route = route.firstChild;
      if (route.snapshot.params['pathwayId'] !== undefined) {
        return route.params;
      }
    }
    return route.params;
   ...
   * This is needed because the service is a singleton and the injected route is the root.
   */
