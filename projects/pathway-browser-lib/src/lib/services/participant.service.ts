...
export class ParticipantService {
  ...
  constructor(private http: HttpClient, private dataState: DataStateService, private config: PathwayBrowserConfigService) {

  }


  getParticipants(stId: string): Observable<Participant[]> {
    const url = `${this.config.CONTENT_SERVICE}/data/participants/${stId}`;
  ...
  getReferenceEntities(stId: string): Observable<ReferenceEntity[]> {
    const url = `${this.config.CONTENT_SERVICE}/data/participants/${stId}/referenceEntities`;
