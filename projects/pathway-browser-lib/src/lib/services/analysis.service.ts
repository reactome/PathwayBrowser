...
export class AnalysisService {
  ...
  analyse(data: string, params?: Partial<Analysis.Parameters>, clearFilters = true): Observable<Analysis.Result> {
    ...
    return this.http.post<Analysis.Result>(`${this.config.host}/AnalysisService/identifiers/${params?.projectToHuman ? 'projection' : ''}`, data, {params}).pipe(
  ...
  analyseSpecies(species: Species, params?: Partial<Analysis.Parameters>, clearFilters = true): Observable<Analysis.Result> {
    this.isLoading.set(true)
    return this.http.get<Analysis.Result>(`${this.config.host}/AnalysisService/species/homoSapiens/${species.dbId}`, {params}).pipe(
      tap(result => this.result.set(result)),
      tap(result => this.resultResource.set(result)),
      tap(result => clearFilters && this.clearFilters()),
      tap(result => this.state.analysis.set(result.summary.token)),
  ...
  analyseFromUrl(url: string, params?: Partial<Analysis.Parameters>, clearFilters = true): Observable<Analysis.Result> {
    this.isLoading.set(true)
    return this.http.post<Analysis.Result>(`${this.config.host}/AnalysisService/identifiers/url${params?.projectToHuman ? '/projection' : ''}`, url, {params}).pipe(
      tap(result => this.result.set(result)),
      tap(result => this.resultResource.set(result)),
      tap(result => clearFilters && this.clearFilters()),
      tap(result => this.state.analysis.set(result.summary.token)),
  ...
  loadAnalysis(token?: string, params?: Partial<Analysis.Parameters>): Observable<Analysis.Result> {
    cleanObject(params || {})
    return this.http.get<Analysis.Result>(`${this.config.host}/AnalysisService/token/${token || this.state.analysis()}`, {params})
  }

  foundEntities(pathway: string, token?: string, resource: Analysis.Resource = 'TOTAL'): Observable<Analysis.FoundEntities> {
    return this.http.get<Analysis.FoundEntities>(`${this.config.host}/AnalysisService/token/${token || this.state.analysis()}/found/all/${pathway}`, {
      params: {resource}
    }).pipe(
      catchError(() => of({
        pathway,
  ...
  pathwaysResults(pathwayIds: number[] | string[], token?: string, resource: Analysis.Resource = 'TOTAL'): Observable<Analysis.Pathway[]> {
    ...
    return this.http.post<Analysis.Pathway[]>(`${this.config.host}/AnalysisService/token/${token || this.state.analysis()}/filter/pathways`, pathwayIds.join(','), {
      params: {resource}
    }).pipe(
      catchError(() => of([]))
    )
  ...
  getHitReactions(pathwayId: string, token: string, params?: Partial<Analysis.Parameters>) {
    ...
    return this.http.get<number[]>(`${this.config.host}/AnalysisService/token/${token}/reactions/${pathwayId}`, {params})
  }
  ...
  notFound(token: string, pagination: Pagination): Observable<NotFoundIdentifier[]> {
    return this.http.get<NotFoundIdentifier[]>(`${this.config.host}/AnalysisService/token/${token}/notFound`, {
      params: pagination
    })
  }
