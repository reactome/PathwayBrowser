...
export class DataStateService {
  ...
  getReacfoamFlagging(query: string, species: string): Observable<Partial<FlaggingResult>> {
    return this.http.get<FireworksFlagResult>(`${this.config.CONTENT_SERVICE}/search/fireworks/flag`, {
  ...
  getDiagramFlagging(diagram: string, query: string): Observable<Partial<FlaggingResult>> {
    return this.http.get<DiagramFlagResult>(`${this.config.CONTENT_SERVICE}/search/diagram/${diagram}/flag`, {
  ...
  constructor(private state: UrlStateService, private http: HttpClient, private species: SpeciesService, private config: PathwayBrowserConfigService) {
  ...
  fetchEnhancedData<T extends DatabaseObject>(id: string | number | null, params?: Partial<{
    ...
    let url = `${this.config.CONTENT_SERVICE}/data/query/enhanced/v2/${id}`;
  ...
  fetchAncestors(stId: string | undefined, path: string[]): Observable<Pathway[]> {
    let url = `${this.config.CONTENT_SERVICE}/data/event/${stId}/ancestors`;
