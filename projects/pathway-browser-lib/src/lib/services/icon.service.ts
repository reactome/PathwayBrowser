...
// Not in used for now, leave here for future use
  ...
  loadIcon(id: string): Observable<string> {
    return this.http.get(`${this.config.host}/icon/${id}.svg`, {responseType: 'text'});
  }

  fetchIcon(identifier: string): Observable<string | null> {
    return this.http.get<Search.Icon.Result>(`${this.config.CONTENT_SERVICE}/search/query?query=${identifier}&types=Icon`).pipe(
      map(response =>
